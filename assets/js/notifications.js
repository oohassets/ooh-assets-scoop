...
function parseDate(v) {
  ...
  if (p.length >= 3)
    return new Date(p[2] || new Date().getFullYear(), p[0] - 1, p[1]);
  return null;
}
...
function uid() {
  return window.__currentUser?.uid || "anon";
}
...
function getReadSet() {
  try {
    return new Set(
      JSON.parse(localStorage.getItem(`notif_read_${uid()}`) || "[]"),
    );
  } catch {
    return new Set();
  }
...
function saveReadSet(set) {
  try {
    localStorage.setItem(`notif_read_${uid()}`, JSON.stringify([...set]));
  } catch {}
}
...
function getClearedAt() {
  try {
    return parseInt(localStorage.getItem(`notif_cleared_${uid()}`) || "0", 10);
  } catch {
    return 0;
  }
}
...
function clearAll() {
  try {
    localStorage.setItem(`notif_cleared_${uid()}`, String(Date.now()));
  } catch {}
  allNotifs = [];
  updateBadge();
  renderList();
...
async function checkSWUpdate() {
  ...
  try {
    ...
    if (!reg) {
      console.warn("[SCOOP SW] Registration not available");
      return;
    }
    console.log(
      `[SCOOP SW] Active: ${reg.active?.state ?? "none"} | Waiting: ${!!reg.waiting} | Installing: ${!!reg.installing}`,
    );
    ...
    if (reg.waiting) {
      injectSWNotif(
        ...
        "A new version of SCOOP is ready.",
        "system_update_alt",
        "update",
      );
    }
    ...
    reg.addEventListener("updatefound", () => {
      ...
      sw.addEventListener("statechange", () => {
        ...
        if (
          ...
          notified = true;
          console.log(
            `[SCOOP SW] New SW detected via statechange: ${sw.state}`,
          );
          injectSWNotif(
            "App Update Available",
            "A new version of SCOOP is ready.",
  ...
  try {
    ...
    if (clearedAt > 0) {
      allNotifs = allNotifs.filter(
        (n) => n.date && n.date.getTime() > clearedAt,
      );
    }
    ...
    console.log(
      `[SCOOP Notifications] ${allNotifs.length} loaded, ${unread} unread`,
    );
